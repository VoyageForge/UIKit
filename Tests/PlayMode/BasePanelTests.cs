...
{
    /// <summary>
    /// BasePanel lifecycle tests — OnCreate/OnShow/OnHide/OnClose core flow.
    /// Driven through PopupManager, not dependent on UIManager global singleton.
    /// </summary>
    public class BasePanelTests
    {
        ...
        public IEnumerator ShowHideShow_OnCreateOnlyOnce() => UniTask.ToCoroutine(async () =>
        {
            ...
            await _popupManager.ShowAsync((PopupPanel)cached);
        ...
        public IEnumerator DuplicateShow_WhenAlreadyActive_IsNoop() => UniTask.ToCoroutine(async () =>
        {
            ...
            Assert.AreEqual(showCount, _panel.OnShowCount, "Already active: OnShow should not fire again");
        });
    }
}
